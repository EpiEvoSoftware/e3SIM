...
def create_slim_config(all_config):
    ...
    slim_pars["link"] = all_config["GenomeElement"]["trait_prob_link"]["link"]
    slim_pars["alpha_trans"] = all_config["GenomeElement"]["trait_prob_link"][slim_pars["link"]]["alpha_trans"]
    slim_pars["alpha_drug"] = all_config["GenomeElement"]["trait_prob_link"][slim_pars["link"]]["alpha_drug"]
    ...
    slim_pars["surviv_prob"] = all_config["EpidemiologyModel"]["transition_prob"]["surviv_prob"]
    ...
    for param in ["S_IE_rate", "I_R_rate", "R_S_rate", "latency_prob", "E_I_rate", "I_E_rate", "E_R_rate", "sample_rate", "recovery_prob_after_sampling", "surviv_prob"]:
        ...
        if param in ["surviv_prob", "S_IE_rate"] and any(i == 1 or i == 0 for i in slim_pars[param]):
            raise CustomizedError(f"({param}) has to be a list of probabilities within (0,1), not equal to 0 or 1")
    ...
    for param in ["alpha_trans", "alpha_drug"]:
        if not isinstance(slim_pars[param], numbers.Number):
            raise CustomizedError(f"({param}) has to be a numerical value")
        if slim_pars[param] <= 0:
            raise CustomizedError(f"({param}) has to be positive")
        out_config.write(f"{param}:{str(slim_pars[param])}\n")
...
def create_slim_script(slim_pars):
    ...
    code_path = os.path.join(os.path.dirname(__file__), "slim_scripts_2025")
    ...
    if slim_pars["use_genetic_model"] == False:
        ...
        if any(idx > 0 for idx in slim_pars["transmissibility_effsize"]):
            if slim_pars["link"]=="logit":
                append_files(os.path.join(code_path, "transmission_additive_logit.slim"), mainslim_path)
            elif slim_pars["link"]=="cloglog":
                append_files(os.path.join(code_path, "transmission_additive_cloglog.slim"), mainslim_path)
        # else:
    ...
    if any(idx != 0 for idx in slim_pars["drugresistance_effsize"]):
        if slim_pars["link"]=="logit":
            append_files(os.path.join(code_path, "Infected_process_additive_logit.slim"), mainslim_path)
        elif slim_pars["link"]=="cloglog":
            append_files(os.path.join(code_path, "Infected_process_additive_cloglog.slim"), mainslim_path)
