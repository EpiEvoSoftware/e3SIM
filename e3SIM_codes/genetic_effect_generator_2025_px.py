...
class Calibrator:
    def __init__(self, df_eff, V_target, default_V=1.0, **kwargs):
        self.df_eff = df_eff
...
class EffectGenerator:
    ...
    def _build_effect_df(self) -> pd.DataFrame:
        if self.cfg.method == "gff":
            ...
            df = self._apply_redo(df, self.cfg.trait_num)
    ...
    def _calibrate_linkslope(self, Rs: np.array, link_type: str, var_em: np.array, trait_num: dict) -> np.array:
        ...
        if all([i > 0 for i in SD_em]):
            ...
            alpha_trans_list = [float(x) for x in np.atleast_1d(alpha_trans)]
            alpha_drug_list  = [float(x) for x in np.atleast_1d(alpha_drug)]
...
def main():
    ...
    parser.add_argument('-link', action='store',dest='link', type=str, required=False, help="Link type", default="logit")
