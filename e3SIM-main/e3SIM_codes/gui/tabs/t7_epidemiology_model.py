...
class EpidemiologyModel(TabBase):
    ...
    def load_page(self):
        ...
        self.render_image(hide, os.path.join(os.path.dirname(os.path.abspath(__file__)), "../assets/t7.png"), 500, 300, 1, 1, 17)
    ...
    def render_slim_path_label(self, hide, column, columnspan, frow):
        text = "Random number generator for SLiM simulation replicates (Optional)"
...
####################################
    ...
    def render_drug_resistance(self, hide, column, columnspan, frow):
        ...
        component = EasyEntry(
            ...
            self.scrollable_frame, column, frow, 'list integer', hide, columnspan,
            labtext="Has to be a list whose length is the number of epochs specified above, and each entry represents the parameter for one epoch (in time-order). Each entry has to be an integer within the range specified in the \"genome element\" tab. For example, specifying \"0\" would mean no genetic effect for drug-resistance in this epoch; specifying \"1\" would mean using Transmissibility_1 for this epoch."
        )
        self.visible_components.add(component)
        return component
